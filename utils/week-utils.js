...
function getWeekDayList(date,selectWeek) {
    ...
    for (var i = 0; i < 7; i++) {
      ...
      weekDay.eg = getEg(mondayTime + i * 24 * 60 * 60 * 1000);
      weekDay.ym = getYearMonth(mondayTime + i * 24 * 60 * 60 * 1000);
        weekDay.date = (function (t) { 
            var time = new Date(t);
            return time.getFullYear() + '/' + (time.getMonth() +1) + '/' + time.getDate();
         })(mondayTime + i * 24 * 60 * 60 * 1000);
  ...
  function getWeek(res) {
   ...
   function getEg(res) {
    ...
    return list[new Date(y + '/' + m + '/' + d).getDay()];
  }
